...
namespace :data do
  namespace :cashflows do
    ...
    task create: :environment do
      ...
      pre_incomes = parse_body(cashflow_incomes('nfreeman'))['incomes'].size
      test_income_data.values.each do |income|
        create_cashflow_income('nfreeman',
                               income.symbolize_keys)
      end

      post_incomes = parse_body(cashflow_incomes('nfreeman'))['incomes'].size
      puts "# of incomes before: #{pre_incomes}"
      puts "# of incomes after: #{post_incomes}"

      pre_bills = parse_body(cashflow_bills('nfreeman'))['bills'].size
      test_bill_data.values.each do |bill|
        create_cashflow_bill('nfreeman',
                               bill.symbolize_keys)
      end

      post_bills = parse_body(cashflow_bills('nfreeman'))['bills'].size
